...
- Web/desktop UI parity: MAVLink Inspector, a read-only Parameters view, and Flight Log Analysis have shipped on web. Web's Inspector decodes 8 message types (HEARTBEAT + 7 more); the long tail of less-common messages needs `@jawji/mavlink-ts` actually **published to npm** — the repo (github.com/utachicodes/mavlink-ts) is extracted and publish-ready, but nobody has run `npm publish` against it yet. Parameters is view-only — no editing/writing. Flight Log Analysis has no field graphs, flight summary stats, or AI analysis layer. Calibration, Firmware, SITL, and CLI Terminal remain intentionally desktop-only (native hardware access a browser can't do) — per explicit direction, these are not being ported to web.
