...
- **A new "Open Source Packages" page on jawji-gcs** (`/packages`, linked from the homepage nav), listing `@jawji/mavlink-ts` and `mediamtx-toolkit` with what each does and what they're used for inside JAWJI. Found while building this: jawji-orchestrator has **no public mention anywhere on jawji.space** — not on `/software`, not on the homepage, nowhere a customer could discover it exists to buy it. Not fixed yet; flagged for a deliberate decision on positioning/pricing copy before adding it anywhere public.
