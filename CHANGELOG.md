...
- **jawjideck desktop is now wired into jawji-gcs's licensing core** — a new Licensing tab under Settings signs in via jawji-gcs's `/desktop-auth` page (opened in the system browser) and a `jawji://auth-callback` deep link, backed by a real Firebase Auth session (`signInWithCustomToken`) in the renderer. Once signed in it shows subscription/license status, activates a 13-character code, starts checkout for jawji-orchestrator, and lists the Jawji Intelligence module catalog with per-module access/buy state — this is the first UI in any Jawji product that actually talks to the licensing backend added above.
- **jawji-orchestrator now enforces its own license instead of being proprietary in name only.** Adds a bundled CLI (`login`, `activate <code>`, `whoami`): `login` opens a browser to jawji-gcs's `/desktop-auth` and receives a credential back over a one-shot localhost HTTP callback (works over an SSH port-forward, no OS protocol handler needed). `activate` binds a purchased code to *this companion computer* specifically (via `/etc/machine-id`, not the flight controller — an FC can be swapped without re-licensing the box running Orchestrator). `createOrchestrator`'s new optional `license` config makes `start()` refuse to run without an active license bound to that device; omitting it (existing integrations, local dev) skips the check entirely, so this doesn't break anything already built against the package.
- `jawji-gcs`'s `/desktop-auth` handoff now accepts a validated `returnTo` target (the desktop app's `jawji://` callback, or a loopback HTTP callback for the orchestrator CLI above), instead of being hardcoded to only support desktop.
- **Desktop's Licensing tab now caches entitlements for offline use**, closing the gap called out immediately below in the previous entry: a live fetch's `{snapshot, token}` is persisted to disk (`electron-store`, via new `licensing:cache-get`/`licensing:cache-set` IPC handlers), and a failed fetch that looks like a real connectivity failure (not a 401/4xx from the server) falls back to the last cached snapshot for the signed-in uid. Cached data older than 7 days is still shown, never hard-locked, but flagged "needs re-verification" in the UI.
- **First slice of web/desktop UI parity: a MAVLink Inspector on jawji-gcs's web dashboard** (`/dashboard/mavlink`, below the connection card), mirroring jawjideck desktop's Inspector — live message table (name, sysid/compid, count, Hz, B/s), pause/resume, clear, name/sysid/compid filters, expandable rows. Only HEARTBEAT is field-decoded on the web today (jawji-gcs doesn't have desktop's shared `@jawji/mavlink-ts` dialect decoder available yet); every other message shows raw hex bytes instead, called out honestly in both the in-app UI and the docs rather than pretending it's fully decoded. The MAVLink page itself was also completely unreachable from the sidebar nav before this — added it.
- **Web Inspector now decodes 7 more common message types** (SYS_STATUS, SYSTEM_TIME, GPS_RAW_INT, ATTITUDE, GLOBAL_POSITION_INT, VFR_HUD, STATUSTEXT), copying field layouts and CRC_EXTRA values from `@jawji/mavlink-ts`'s generated definitions rather than re-deriving them from memory — caught and fixed one transcription slip (GPS_RAW_INT's CRC_EXTRA briefly copied as its message id) before it shipped.
- **A read-only Parameters view on the web dashboard**, the second UI parity slice: sends `PARAM_REQUEST_LIST` and displays the resulting `PARAM_VALUE` stream in a searchable, CSV-exportable table with a live "N/total received" count. This is the first time jawji-gcs's MAVLink bridge sends anything to the vehicle (previously receive-only) — scoped narrowly to `Transport.send()` plus one read-only request builder; no parameter-write path exists.
- **`@jawji/mavlink-ts` extracted into its own public repo**, [github.com/utachicodes/mavlink-ts](https://github.com/utachicodes/mavlink-ts) — the full generated `common.xml` message registry, streaming parser, serializers, and MAVLink 2 signing, previously workspace-only inside this monorepo. Verified it builds clean and all 46 tests pass in the standalone repo before pushing.
- **`@jawji/mavlink-ts@0.1.0` published to npm** (by the user, not this agent — publishing needs real credentials, and a token pasted in chat was declined per this session's standing policy). jawji-gcs now depends on it directly: the web Inspector field-decodes every message in the `common.xml` dialect — hundreds of types — instead of the 8-message hand-written subset from the entry above, which is now deleted as fully redundant.
- **Flight Log Analysis on the web dashboard**, the third UI parity slice and the one specifically requested: parses ArduPilot `.bin` dataflash logs entirely client-side (never uploaded to the server), runs the same 8 health checks as desktop (vibration, GPS, battery, compass, EKF, failsafe, power, flight modes), and browses raw messages by type. The parser (`lib/dataflash/*` in jawji-gcs) is vendored from `@jawji/dataflash-parser` rather than depended on — it's pure TypeScript with zero Node-specific APIs, so a straight copy is correct, not a reimplementation. No field graphs, flight summary stats (max altitude/speed/distance), or AI analysis layer yet — health checks and raw message browsing only.
- **`mediamtx-toolkit@0.1.0` published to npm** (github.com/utachicodes/mediamtx-toolkit) — a typed `MediaServer` API over MediaMTX's binary/process lifecycle and Control API (install, start, crash-detect-and-restart, hot-reload config, list paths), currently "Layer 0: Core" per its own README (no per-path stream management, recording, or clustering yet). Manages the MediaMTX process on companion computers running Jawji Controller.
- **A new "Packages" page on jawji-gcs** (`/packages`, linked from the homepage nav), split into two sections: **Open Source** (`@jawji/mavlink-ts`, `mediamtx-toolkit` — MIT, npm/source links) and **Licensed** (`@jawji/orchestrator` — what it does, how its per-device licensing works, a link to sign in and activate). This closes jawji-orchestrator's total lack of public visibility on jawji.space (found while building the open-source-only version of this page) without committing to pricing-tier copy yet — that's still a separate, deliberate decision. No public link to the orchestrator source repo since it's private and would 404 for visitors.
...
- **A bug in the desktop Licensing tab's own first version:** it passed the connected flight controller's board UID as the `hardwareId` for orchestrator license activation. Wrong device entirely — Orchestrator runs on the companion computer, not the FC. Desktop activation no longer sends a hardwareId; orchestrator codes get bound correctly via `jawji-orchestrator activate` instead (see above).
- Companion dashboard's "Jawji Controller Not Connected" install command still pointed at a `raw.githubusercontent.com` URL instead of `jawji.space/install.sh`.
- Link checker was flagging `github.com/utachicodes/jawji-orchestrator` and `github.com/utachicodes/jawji-gcs` as broken — both are private repos, so a 404 to an unauthenticated crawler is expected, not a real broken link. Excluded from `.lychee.toml`.
- CodeQL analysis was failing every run on "Resource not accessible by integration" after a successful scan — the job's `GITHUB_TOKEN` was missing `actions: read`, which codeql-action v3 needs for its diff-range/telemetry follow-up call. Added.
- A latent crash: desktop's `lib/firebase.ts` called `initializeApp`/`getAuth` with empty-string config when `VITE_FIREBASE_*` env vars weren't set, which throws `auth/invalid-api-key` immediately at module load — not a graceful "licensing unavailable" state, the whole renderer would have failed to start. Now falls back to obvious placeholder values (matching jawji-gcs's own `lib/firebase.ts`), with every real usage still gated on `firebaseConfigured`.

### Known gaps
- The desktop/orchestrator sign-in flow above is implemented and unit-tested at the logic layer, but the full browser → deep-link/localhost-callback → session round trip has not been run end-to-end yet — do that before relying on it in the field.
- Jawji Intelligence has no real inference behind it yet (`lib/server/intelligence/inference-provider.ts` is a stub); the catalog/access/checkout plumbing above is real, the AI itself is not.
- Web/desktop UI parity: MAVLink Inspector, a read-only Parameters view, and Flight Log Analysis have shipped on web. The Inspector's message-decoding gap is now closed — `@jawji/mavlink-ts` is published and the web Inspector decodes the full `common.xml` dialect. Parameters is still view-only — no editing/writing. Flight Log Analysis has no field graphs, flight summary stats, or AI analysis layer. Calibration, Firmware, SITL, and CLI Terminal remain intentionally desktop-only (native hardware access a browser can't do) — per explicit direction, these are not being ported to web.
- Offline caching now covers desktop's Licensing tab only. jawji-orchestrator's own license check is always live at `start()` (by design — it's meant to gate mission start, not run cached) and has no offline grace period yet if `jawji-gcs` is unreachable at boot.

## [0.0.40] - 2026-07-09

### Added
- **jawji-orchestrator**, a new standalone package published separately at [github.com/utachicodes/jawji-orchestrator](https://github.com/utachicodes/jawji-orchestrator) (`@jawji/orchestrator` on npm). Runs on a companion computer as its own process with a direct MAVSDK connection to the flight controller, independent of whether a Jawji GCS is connected at all. Ships a `VisionAssistMode` framework with a first mode, `LandingZoneCheckMode`: on entering LAND mode it holds the vehicle, captures a frame, asks an integrator-supplied vision-language model whether the site is safe, and if not, gates any repositioning behind an external confirm by default (`confirmPolicy: 'gated'`) rather than acting unattended. There is no official MAVSDK Node.js client on npm — verified directly against the `mavlink` GitHub org and the npm registry before building this — so the package generates its own gRPC client from the official `mavlink/MAVSDK-Proto` definitions using `@grpc/grpc-js` (pure JavaScript, no native addon, which matters for cross-compiling to Jetson/Pi ARM). Not wired into Jawji desktop or the Controller at the time of this release — see Unreleased above for the licensing wiring that connects them.
