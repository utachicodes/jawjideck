...
- Web/desktop UI parity is *started*, not close to done — MAVLink Inspector and a read-only Parameters view shipped this round. Web's Inspector decodes 8 message types (HEARTBEAT + 7 more); the long tail of less-common messages needs `@jawji/mavlink-ts` actually published (it's currently workspace-only inside this monorepo, not consumable from jawji-gcs's separate repo). Parameters is view-only — no editing/writing, and desktop still has Calibration, Flight Logs, Firmware, SITL, and CLI Terminal with no web equivalent at all.
