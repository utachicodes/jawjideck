...
- Web/desktop UI parity is *started*, not close to done — one screen (MAVLink Inspector) shipped this round. Web's Inspector only decodes HEARTBEAT; full field decoding for every message type needs `@jawji/mavlink-ts` actually published (it's currently workspace-only inside this monorepo, not consumable from jawji-gcs's separate repo).
