...
export function LogListPanel() {
  ...
  const handleOpenRecent = async (log: RecentLog) => {
    ...
    const result = await window.electronAPI.logReadFile(log.path);
    if (result) {
      await window.electronAPI.logRecentAdd({ path: log.path, name: log.name, size: result.data.length });
      await parseAndAnalyze(result.data, result.path);
      window.electronAPI.logRecentGet().then(setRecentLogs);
    } else {
      // File no longer exists - refresh list
      window.electronAPI.logRecentGet().then(setRecentLogs);
    }
