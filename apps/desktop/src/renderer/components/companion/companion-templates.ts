...
export const COMPANION_TEMPLATES: CompanionTemplate[] = [
  // ── ESP32 Templates ──────────────────────────────────────────
  {
    ...
    description: 'Replace SiK radio with WiFi telemetry. Connect QGroundControl, Mission Planner, or Jawji wirelessly.',
    board: 'esp32',
  ...
  // ── Raspberry Pi Templates ───────────────────────────────────
  {
    ...
    category: 'Telemetry',
    flashMethod: 'image',
    installCommand: 'curl -fsSL https://jawji.space/install.sh | sudo bash -s -- basic',
    features: ['mavlink-router daemon', 'WiFi access point (NetworkManager)', 'Multiple GCS connections', 'Jawji Controller pre-installed', 'Auto-start on boot'],
    requirements: ['Raspberry Pi with WiFi', 'MicroSD card (8GB+)', 'UART or USB connection to FC', '5V 3A BEC', 'Raspberry Pi OS Bookworm or later (NetworkManager)'],
    projectName: 'mavlink-router + NetworkManager',
  },
  {
    id: 'pi-video-telemetry',
    name: 'Video + Telemetry',
    description: 'MediaMTX media server + MAVLink routing. RTSP/RTMP/HLS/WebRTC out — Jawji\'s Camera panel connects directly over WebRTC.',
    board: 'raspberry-pi',
    boardVariants: ['Pi 4', 'Pi 5'],
    category: 'Video',
    flashMethod: 'image',
    installCommand: 'curl -fsSL https://jawji.space/install.sh | sudo bash -s -- vision',
    features: ['MediaMTX (RTSP/RTMP/HLS/WebRTC, for QGC/VLC/browsers)', 'WebRTC (WHEP) feeds straight into Jawji\'s Camera panel', 'MAVLink routing', 'WiFi AP', 'Jawji Controller'],
  ...
  },
  {
    ...
    category: 'Autonomy',
    flashMethod: 'image',
    installCommand: 'curl -fsSL https://jawji.space/install.sh | sudo WITH_CONTROLLER=1 WITH_MAVLINK=1 WITH_MAVSDK=1 WITH_YOLO=0 WITH_WIFI_AP=0 WITH_MEDIAMTX=0 bash',
    features: ['MAVSDK Python venv', 'MAVLink routing', 'Mission script examples', 'Jawji Controller', 'mavlink-router + controller auto-start on boot (mission scripts run manually, by design)'],
  ...
  // ── Jetson Templates ─────────────────────────────────────────
  {
    ...
    category: 'Autonomy',
    flashMethod: 'script',
    installCommand: 'curl -fsSL https://jawji.space/install.sh | sudo bash -s -- ai',
    features: ['YOLO object detection (GPU, Ultralytics)', 'MAVSDK integration', 'MAVLink routing', 'Camera pipeline example', 'Jawji Controller', 'ROS2 optional (not automated)'],
