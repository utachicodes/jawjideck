-- Jawji Commands - heartbeat + MAV_CMD_USER_1 sub-command dispatcher.
--
...
gcs:send_text(6, string.format('Jawji commands v%.1f loaded', SCRIPT_VERSION))
...
do
  ...
  mavlink_ready = ok
  if not ok then
    gcs:send_text(4, 'Jawji: mavlink init FAILED: ' .. tostring(err))
  else
    gcs:send_text(6, 'Jawji: mavlink rx ready (COMMAND_INT, USER_1 blocked)')
  end
end
...
local function start_orbit(c)
  ...
  gcs:send_text(6, string.format(
    'Jawji ORBIT: r=%.1f alt=%.1f v=%.1f revs=%s',
...
local function start_spiral(c)
  ...
  cmd_state.active        = true
  gcs:send_text(6, string.format(
    'Jawji SPIRAL: r=%.1f alt %.1f→%.1f climb=%.1f v=%.1f',
...
local function ensure_guided()
  ...
  if not ok or not result then
    ...
    gcs:send_text(4, string.format(
      'Jawji: set_mode(GUIDED) failed (was mode=%s armed=%s)',
      tostring(mode), tostring(arming:is_armed())))
    return false
  end
  gcs:send_text(6, string.format('Jawji: switched to GUIDED (was %s)', tostring(mode)))
  return true
end
...
local function safe_set_target_location(target)
  ...
  if not ok then
    stats.errors = stats.errors + 1
    gcs:send_text(4, 'Jawji: set_target_location threw: ' .. tostring(result))
  ...
  if stats.set_target_ok == 0 then
    gcs:send_text(4, string.format(
      'Jawji: set_target_location returned false (mode=%s armed=%s)',
...
local function safe_set_pos_ned_with_yaw(target_loc, yaw_deg)
  ...
  if not ok then
    stats.errors = stats.errors + 1
    gcs:send_text(4, 'Jawji: set_target_pos_NED threw: ' .. tostring(result))
    return false
  end
  if result then
    stats.set_target_ok = stats.set_target_ok + 1
  ...
  if stats.set_target_ok == 0 then
    gcs:send_text(4, string.format(
      'Jawji: set_target_pos_NED returned false (mode=%s armed=%s)',
      tostring(vehicle:get_mode()), tostring(arming:is_armed())))
  end
  return false
end
...
local function step_orbit_or_spiral(now_ms)
  ...
  if not cmd_state.approach_done then
    ...
    if math.abs(dist_to_center - r) <= APPROACH_TOLERANCE_M then
      ...
      gcs:send_text(6, 'Jawji: on ring, beginning orbit')
  ...
  if cmd_state.sub_id == SUB_CMD_ORBIT
    ...
    cmd_state.active = false
    gcs:send_text(6, string.format('Jawji ORBIT: complete (%d revs)',
  ...
  if cmd_state.sub_id == SUB_CMD_SPIRAL and cmd_state.climb_mps > 0 then
    ...
    if math.abs(step) >= math.abs(remaining) then
      ...
      cmd_state.climb_mps = 0
      gcs:send_text(6, string.format('Jawji SPIRAL: target alt %.1fm reached, holding orbit',
...
local function step_watchtower(now_ms)
  ...
  if not cmd_state.approach_done then
    ...
    if dist <= APPROACH_TOLERANCE_M then
      ...
      cmd_state.approach_done = true
      gcs:send_text(6, 'Jawji WATCHTOWER: at point, beginning rotation')
...
local function step_climb_rtl(_now_ms)
  ...
  if math.abs(current_rel_alt - cmd_state.target_alt_m) < 2 then
    ...
    if ok then
      gcs:send_text(6, string.format('Jawji CLIMB_RTL: at %.1fm, switching to RTL', current_rel_alt))
      cmd_state.active = false
    end
  end
end
...
local function step_reveal(now_ms)
  ...
  if cmd_state.walked_m >= cmd_state.walk_total_m then
    gcs:send_text(6, 'Jawji REVEAL: complete, holding')
    cmd_state.active = false
  end
end
...
local function step_strafe(now_ms)
  ...
  if not cmd_state.approach_done then
    ...
    if d <= APPROACH_TOLERANCE_M then
      cmd_state.approach_done = true
      gcs:send_text(6, 'Jawji STRAFE: at start, beginning dolly')
    else
  ...
  if cmd_state.walked_m >= cmd_state.walk_total_m then
    gcs:send_text(6, 'Jawji STRAFE: complete, holding')
    cmd_state.active = false
  end
end
...
local function step_land_at(_now_ms)
  ...
  if d <= APPROACH_TOLERANCE_M then
    ...
    if ok then
      gcs:send_text(6, string.format('Jawji LAND_AT: at point (%.1fm), switching to LAND', d))
      cmd_state.active = false
    end
  end
end
...
local function start_watchtower(c)
  ...
  cmd_state.approach_done   = false
  cmd_state.active          = true
  gcs:send_text(6, string.format('Jawji WATCHTOWER: alt=%.1f rate=%.1f°/s',
    c.z, rate))
end
...
local function start_climb_rtl(c)
  local here = ahrs:get_location()
  if here == nil then
    gcs:send_text(4, 'Jawji CLIMB_RTL: ahrs:get_location returned nil')
    return
  end
  ...
  cmd_state.last_step_ms = millis():tofloat()
  cmd_state.active       = true
  gcs:send_text(6, string.format('Jawji CLIMB_RTL: climbing to %.1fm then RTL', c.z))
end
...
local function start_reveal(c)
  ...
  cmd_state.last_step_ms  = millis():tofloat()
  cmd_state.active        = true
  gcs:send_text(6, string.format('Jawji REVEAL: pullback=%.1f Δalt=%.1f v=%.1f',
...
local function start_strafe(c)
  local here = ahrs:get_location()
  if here == nil then
    gcs:send_text(4, 'Jawji STRAFE: ahrs:get_location nil')
    return
  end
  local target_loc = build_center(c, c.z)
  local target_to_vehicle_rad = target_loc:get_bearing(here)
  if target_to_vehicle_rad == nil then
    gcs:send_text(4, 'Jawji STRAFE: get_bearing nil')
    return
  end
  ...
  cmd_state.approach_done = false
  cmd_state.active        = true
  gcs:send_text(6, string.format('Jawji STRAFE: offset=%.1f length=%.1f v=%.1f',
    offset, length, speed))
end
...
local function start_land_at(c)
  local here = ahrs:get_location()
  if here == nil then
    gcs:send_text(4, 'Jawji LAND_AT: ahrs:get_location nil')
    return
  end
  ...
  cmd_state.last_step_ms  = millis():tofloat()
  cmd_state.active        = true
  gcs:send_text(6, string.format('Jawji LAND_AT: flying to point @ %.1fm then LAND', approach_alt))
end
...
local function handle_command_int(cmd, _chan)
  ...
  if sub_id == SUB_CMD_STOP then
    if cmd_state.active then
      gcs:send_text(6, 'Jawji: STOP — releasing vehicle')
    end
    cmd_state.active = false
    cmd_state.sub_id = -1
    return
  ...
  if starter == nil then
    stats.errors = stats.errors + 1
    gcs:send_text(4, 'Jawji: unknown sub-command id ' .. tostring(sub_id))
    return
  end
  ...
  if not ok then
    stats.errors = stats.errors + 1
    gcs:send_text(4, 'Jawji: starter threw: ' .. tostring(err))
  end
end
